...
// Still don't understand the websocket? click here: https://imabotplayer.github.io/url-transformer/index.html
...
class Bot {
    ...
    handleBuffer(buffer) {
        ...
        switch (reader.readUint8()) {	
            case 17:		
                this.viewX = reader.readFloat32();	
				this.viewY = reader.readFloat32();	
    ...
    updateViewportEntities(reader) {
        ...
        while (true) {
			...
			entity.x = this.getX(entity.x) //fix for ffa/exp
			entity.y = this.getY(entity.y) //fix for ffa/exp			
    ...
    updateOffset(reader) {
        ...
        if (~~(right - left) === 14142 && ~~(bottom - top) === 14142) {
			this.offsetX = 7071 - right;
			this.offsetY = 7071 - bottom;	            
	...
	GhostFix(){
		if(!this.ghostFixed && this.mapOffsetFixed && this.ghostCells.length!=0 && Math.abs(user.ghostX)>100 && Math.abs(user.ghostY) >100) {
			this.fixX = /*Math.round*/(user.ghostX /(this.ghostCells[0].x+this.offsetX))<0?-1:1;
            this.fixY = /*Math.round*/(user.ghostY /(this.ghostCells[0].y+this.offsetY))<0?-1:1;
			this.ghostFixed = true	
        }					
	}
    getX(x) {
      if(this.ghostFixed && this.mapOffsetFixed && user.offsetX != 0) {
		  return ((x + this.offsetX) * this.fixX - user.offsetX)
      }
	  else return x;
    }
    getY(y) {
      if(this.ghostFixed && this.mapOffsetFixed && user.offsetY != 0) {
		 return ((y + this.offsetY) * this.fixY - user.offsetY)
      }
	  else return y;
    }
    convertX(x) { //is used only for SendPosition
		if(this.ghostFixed && this.mapOffsetFixed && user.offsetX != 0) {
			return ~~((x + user.offsetX) * this.fixX - this.offsetX)
		}
		else return x;		
    }    
    convertY(y) {
		if(this.ghostFixed && this.mapOffsetFixed && user.offsetY != 0) {
			return ~~((y + user.offsetY) * this.fixY - this.offsetY)	
		}
		else return y;	
    } 	
	...
	sendPosition(x, y){
		...
		//this.send(buffers.move(this.convertX(x), this.convertY(y), this.decryptionKey));
	}
    move() {
        const bot = {
            x: 0,
        ...
        if (user.isAlive && this.followMouse && !stoppingBots && !bots.ai) {
			this.sendPosition(user.mouseX, user.mouseY)
    ...
    ws.on('message', buffer => {
        ...
        switch (reader.readUint8()) {
            ...
            case 6:
                ...
                break
            case 15:
                user.ghostX = reader.readInt32()
                user.ghostY = reader.readInt32()
                break	
            case 16:
                user.offsetX = reader.readInt32()
                user.offsetY = reader.readInt32()
                break					
        }
		}
    })
