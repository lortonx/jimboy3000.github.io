...
function legendmaster(self) {
    ...
    self.master = {
		...
		x-support-proto-version: headers.proto_version,
        getClientVersion: function() {
            ...
            if (null !== self.localStorage.getItem("ogarioProtocolVersion")) {
                this.x-support-proto-version = self.localStorage.getItem("ogarioProtocolVersion");
            }			
            var window = this;
            $.ajax("//agar.io/mc/agario.js", {
                ...
                success: function(sketchContents) {
                    ...
                    if (optionMatch) {
						...
						window.setx-support-proto-version(pluginName2);
                    }
                },
                dataType: "text",
                method: "GET",
        ...
        setx-support-proto-version: function(serverVersion) {			
            if (this.x-support-proto-version != serverVersion) {
                console.log("[Master] Changing protocol version...");
                this.x-support-proto-version = serverVersion;
                self.localStorage.setItem("ogarioProtocolVersion", serverVersion);   
                console.log("[Master] setx-support-proto-version called, reconnecting");
                this.reconnect(true);				
            }
        },		
        ...
        makeMasterRequest: function(_wid_attr, data, callback, timeout_callback, type) {
            ...
            $.ajax("https://" + headers.master_url + "/" + _wid_attr, {
                beforeSend: function(xhr) {
                    return xhr.setRequestHeader("Accept", "text/plain"), xhr.setRequestHeader("Accept", "*/*"), xhr.setRequestHeader("Accept", "q=0.01"), xhr.setRequestHeader("Content-Type", type), xhr.setRequestHeader("x-support-proto-version", master.x-support-proto-version), xhr.setRequestHeader("x-client-version", master.clientVersion), true;
                },
                error: function() {
                    if (timeout_callback) {
                        timeout_callback();
        ...
        makeMasterSimpleRequest: function(key, dataType, success, error) {
            ...
            $.ajax("https://" + headers.master_url + "/" + key, {
                beforeSend: function(xhr) {
                    return xhr.setRequestHeader("x-support-proto-version", master.x-support-proto-version), xhr.setRequestHeader("x-client-version", master.clientVersion), true;
                },
                error: function() {
                    if (error) {
                        error();
