...
jQuery.ajax('http://agar.io/agario.core.js', {
    'success': function(e) {
	  ...
	  /if\(\w\[[\d\w]+\]\|0\)([\w$]+\([\d\w]+,\w\[[\d\w]+\]\|0\);\w=\w;return)/i, '$1', 12, 1), /(if\(\(\w\|0\)==0\|\(\w\[[\d\w]+\]\|0\)==0\){\w\[\w>>2\]\=0;\w\[\w\+[\d\w]+>>2\]=0;\w\[\w\+[\d\w]+>>2\]=0;[\w$]+\(\w,[\d\w]+,15\)}else\s*)([\w$]+\(\w,\w\);)/i, '$2', 13, 1), /(\w\[\w>>2\]=\(\w\[\w>>2\]\|0\)\+1;)([\w$]+\(\w\);)([\w$]+\(\w\);)(if\(!\(\w\[[\d\w]+\]\|0\)\){[\w$]+\(\w,\w\);(\w=\w)})/i, '$1$2$3 if(1){ogario.drawGrid(gameCtx);$5}', 14, 1), 
	  ...
	  /if\(\(\w\[\w\+[\d\w]+>>0\]\|0\)==0\?\(\(\(\w\[\w>>2\]\|0\)\+\(\w\[[\d\w]+\]\|0\)\|0\)>>>0\)%10\|0\|0:0\){\w=\w\[\w>>2\]\|0;[\w$]+\([\d\w]+,\w\|0\)\|0;\w=\w;return}[\w$]+\((\w),\w\);/i, "if(!ogario.drawRemovedCells){ogario.drawCellInfo(gameCtx, $1, cellX, cellY, cellSize, isFood, isVirus, isPlayerCell, skipCell, nick, color, skin !== null);}", 26, 1), /(\w\[\w\+16>>2\]=0;)(\w\[\w>>2\])=([\w$]+\([\d\w]+\)\|0;)(\w\[[\d\w]+\]=\(\w\[[\d\w]+\]\|0\)\+1)/gi, "$1 $2=-1; $4", 27, 1), 
